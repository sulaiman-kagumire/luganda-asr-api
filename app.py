...
crisp_model     = AutoModelForSpeechSeq2Seq.from_pretrained(
    WHISPER_EN,
    torch_dtype=torch.float16 if device == "cuda" else torch.float32,
    low_cpu_mem_usage=True,
    use_safetensors=True,
).to(device)
crisp_pipe = hf_pipeline(
    "automatic-speech-recognition",
    model=crisp_model,
    tokenizer=crisp_processor.tokenizer,
    feature_extractor=crisp_processor.feature_extractor,
    chunk_length_s=30,
...
def transcribe_english(array):
    hf_output = crisp_pipe({"array": array, "sampling_rate": SAMPLE_RATE})
